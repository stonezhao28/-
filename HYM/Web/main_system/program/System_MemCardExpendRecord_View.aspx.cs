...
namespace Web.main_system.program
{
    public partial class System_MemCardExpendRecord_View : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                txtBeginDate.Text = DateTime.Now.AddDays(-7).ToString("yyyy-MM-dd HH:mm:ss");
        ...
        private void BindDataGrid()
        {
            ...
            {
                dt = clsRecord.Bind(Convert.ToDateTime(this.txtBeginDate.Text.Trim()).ToString("yyyy-MM-dd HH:mm:ss"), Convert.ToDateTime(this.txtEndDate.Text.Trim()).AddDays(1).ToString("yyyy-MM-dd HH:mm:ss"), Session["UserID"].ToString());
            }
            else
            {
                dt = clsRecord.Bind(Convert.ToDateTime(this.txtBeginDate.Text.Trim()).ToString("yyyy-MM-dd HH:mm:ss"), Convert.ToDateTime(this.txtEndDate.Text.Trim()).AddDays(1).ToString("yyyy-MM-dd HH:mm:ss"), "");

            }
            DataView dv = new DataView();
            //LogHelper.WriteLog("修改TableName"+ dt.ToString());
            dt.TableName = "Mem_Log";
            //LogHelper.WriteLog("修改后TableName");
            if (dt != null)
            {
                dv.Table = dt;
